...
class MSCropView: UIView, UIGestureRecognizerDelegate {
    ...
    @objc private func handlePan(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        if (imageView.bounds.contains(recognizerFrame!)) {
            ...
            if ((recognizerFrame?.origin.y)! < imageView.bounds.origin.y) {
                ...
                var recognizeFrameCopy = recognizerFrame
                defer { recognizerFrame = recognizeFrameCopy }
                recognizeFrameCopy?.origin.y = imageView.bounds.size.height - (recognizerFrame?.size.height)!
            }
            if ((recognizerFrame?.origin.x)! < imageView.bounds.origin.x) {
                ...
                var recognizeFrameCopy = recognizerFrame
                defer { recognizerFrame = recognizeFrameCopy }
                recognizeFrameCopy?.origin.x = imageView.bounds.size.width - (recognizerFrame?.size.width)!
            }
        }
    ...
    @objc func cropImage() {
        ...
        if(isRounded) {
            ...
            UIGraphicsEndImageContext()
            Photo.setOriginalImage(originalImage: roundedCroppedImage!)
        }
        else {
            Photo.setOriginalImage(originalImage: croppedImage)
        }
